        ...
        Table.TransformRows(#"Named Columns", (r) =>
            Record.TransformFields(r,
                {{"Valuation Source", each if List.Contains({"CMHC", "GENW"}, r[Insurance Company]) then
                    "Error: Valuation source should be " & r[Insurance Company]
                else if _ = "" then
                    "Error: Missing valuation source"
                else _},
                {"Property Valuation", each if _ = "" then "Error: Missing property valuation" else _},
                {"Last Valuation Date", each if _ = "" then "Error: Property valuation date is missing" else _}}))),
...
*/
    ...
    #"Added missing orginal loan amounts" = Table.RemoveColumns(Table.FromRecords(
        Table.TransformRows(#"Added original loan amounts", (r) =>
            Record.TransformFields(r,
                {"Original Loan Amount", each if _ = "" then "Error: Original loan amount is missing" else _}))),
    {"NOTEOPENAMT"}),
    ...
    #"Fill missing security information" = Table.FromRecords(
        Table.TransformRows(#"Update credit scores", (r) => Record.TransformFields(r,
            ...
            {"Property Municipality", each if  _ = "" then
                "Error: Property municipality is missing"
            else _},
            {"Property Province", each if  _ = "" then
                "Error: Property province is missing"
            else _},
            {"Property Postal Code", each if Text.Length( _) < 7 then
                "Error: Property postal code is missing"
            else _}}))),
...
//Corrects missing information for borrower1
     #"Fill missing credit score information" = Table.FromRecords(
        Table.TransformRows(#"Fill missing security information", (r) => Record.TransformFields(r,
            {{"Original Credit Bureau1", each if _ = "" then "Error: Information is missing" else _},
            {"Original Credit Score Type1", each if _ = "" then "Error: Information is missing" else _},
            ...
            else "Error: Information is missing"},
            {"Current Credit Bureau1", each if _ = "" then "Error: Information is missing" else _},
            {"Current Credit Score Type1", each if _ = "" then "Error: Information is missing" else _},
            ...
            {"Current Credit Score Update Date1", each if _ = 0 then "Error: Information is missing" else _}}))),
