...
//Imports person extracts (RCCU and BCU) and applies a source column
    BCUSource = Csv.Document(File.Contents("O:\IFRS9_New\Source Extracts\extractPersBCU.csv"),[Delimiter=",", Columns=100, Encoding=1252, QuoteStyle=QuoteStyle.None]),
    #"Added BCUSource" = Table.AddColumn(BCUSource, "Source", each "BCU"),
    RCCUSource = Table.RemoveRows(Csv.Document(File.Contents("O:\IFRS9_New\Source Extracts\extractPersRCCU.csv"),[Delimiter=",", Columns=100, Encoding=1252, QuoteStyle=QuoteStyle.None]), 0),
...
//Some account numbers are used on both DNA systems.  The below code uses a user defined table to translate the duplicate loan ID's into different values.
    ...
    #"Removed user created columns" = Table.RemoveColumns(#"Fixed duplicated accounts",{"Source", "Type", "TAXRPTFORPERSNBR", "Credit Score", "Run Date", "Translated Loan ID", "ECL Override"}),
